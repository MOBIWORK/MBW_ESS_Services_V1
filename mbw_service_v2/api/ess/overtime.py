...
def create_ot_request(**data):
    try:
        data = dict(data)
        new_ot_rq = frappe.new_doc("ESS ESS Overtime Request")
...
def update_ot_request(**data):
    try:
        ...
        ot_rq = frappe.get_doc("ESS ESS Overtime Request",ot_name)
        ot_date = datetime.fromtimestamp(int(data.get('ot_date'))).date()  if data.get('ot_date') else False
        shift = data.get('shift') if data.get('shift') else False
        ot_start_time = datetime.strptime(data.get('ot_start_time'), "%H:%M:%S").time()  if data.get('ot_start_time') else False
        ot_end_time = datetime.strptime(data.get('ot_end_time'), "%H:%M:%S").time() if data.get('ot_end_time') else False
...
def get_list_ot_request(**params):
    try:
        ...
        list_ot = frappe.db.get_list("ESS ESS Overtime Request",["name","ot_date", "shift", "ot_start_time", "ot_end_time", "ot_approver", "posting_date","employee","suggested_time","status"])
        OtRequest = frappe.qb.DocType("ESS ESS Overtime Request")
...
def delete_ot_request(name):
    try:
        ...
        frappe.delete_doc('ESS ESS Overtime Request',name)
